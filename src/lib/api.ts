...
export interface SousTraitanceOnboardingRequest {
  ...
  sousTraitantEntrepriseNomCommercial?: string;
  sousTraitantEntrepriseActivite?: string;
  sousTraitantEntrepriseAutre?: string;
  contratEnregistre?: boolean;
  volumes?: number;
  quantites?: number;
}
...
// =============================================================================
// Injection certificat legacy (ADMIN_SI uniquement — solution provisoire)
// =============================================================================

export interface LegacyAcCreate {
  code?: string;
  nom: string;
  sigle?: string;
  adresse?: string;
...
export interface LegacyEntrepriseCreate {
  nif: string;
...
export interface LegacyConventionCreate {
  reference: string;
  intitule: string;
  dateSignature?: string;
  bailleurId?: number;
}
export interface LegacyMarcheCreate {
  numeroMarche: string;
  intitule: string;
  dateSignature: string;
  montantContratHt?: number;
}
export interface LegacyRefRef<T> {
  id?: number;
  create?: T;
}
export interface LegacyCertificatPayload {
...
export interface LegacyCertificatInjectionRequest {
  numeroDemandeCorrection?: string;
  dateDepot?: string;
  autoriteContractante: LegacyRefRef<LegacyAcCreate>;
  entreprise: LegacyRefRef<LegacyEntrepriseCreate>;
  convention: LegacyRefRef<LegacyConventionCreate>;
  marche: LegacyRefRef<LegacyMarcheCreate>;
  certificat: LegacyCertificatPayload;
}
export interface LegacyCertificatInjectionResult {
  injectionLegacy: boolean;
  autoriteContractanteId: number;
  entrepriseId: number;
  conventionId: number;
  marcheId: number;
...
export const legacyCertificatApi = {
  inject: (payload: LegacyCertificatInjectionRequest) =>
    apiFetch<LegacyCertificatInjectionResult>("/admin/certificats-legacy/inject", {
      method: "POST",
      body: payload,
    }),
  uploadCorrectionDoc: (demandeId: number, codeDocument: string, file: File) => {
    const form = new FormData();
    form.append("file", file);
    return apiFetch<DocumentDto>(
      `/admin/certificats-legacy/demandes-correction/${demandeId}/documents?codeDocument=${encodeURIComponent(codeDocument)}`,
      { method: "POST", rawBody: form }
    );
  },
  uploadConventionDoc: (conventionId: number, type: string, file: File) => {
    const form = new FormData();
    form.append("file", file);
    return apiFetch<DocumentDto>(
      `/admin/certificats-legacy/conventions/${conventionId}/documents?type=${encodeURIComponent(type)}`,
      { method: "POST", rawBody: form }
    );
  },
  uploadMarcheDoc: (marcheId: number, type: string, file: File) => {
    const form = new FormData();
    form.append("file", file);
    return apiFetch<DocumentDto>(
      `/admin/certificats-legacy/marches/${marcheId}/documents?type=${encodeURIComponent(type)}`,
      { method: "POST", rawBody: form }
    );
  },
  uploadCertificatDoc: (certificatCreditId: number, codeDocument: string, file: File) => {
    const form = new FormData();
    form.append("file", file);
    return apiFetch<DocumentDto>(
      `/admin/certificats-legacy/certificats-credit/${certificatCreditId}/documents?codeDocument=${encodeURIComponent(codeDocument)}`,
      { method: "POST", rawBody: form }
    );
  },
};
