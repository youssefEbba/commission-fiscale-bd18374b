...
const HeroSection = () => {
  ...
  const stats = [
    { value: "2 500+", label: t("hero.stats.treated") },
    { value: "48h", label: t("hero.stats.delay") },
      ...
      <div className="relative mx-auto max-w-7xl px-6">
        <div className="flex flex-col items-center text-center">
          ...
          >
            ...
            <span className="text-xs font-semibold text-gold tracking-wide">{t("hero.badge")}</span>
          </motion.div>
          ...
          >
            {t("hero.title_line1")}
            <br />
            <span className="text-gradient-gold">{t("hero.title_line2")}</span>
          </motion.h1>
          ...
          >
            <Button size="lg" className="bg-gold text-accent-foreground font-semibold text-base px-8 hover:bg-gold-light gap-2" asChild>
              <Link to="/register">{t("hero.cta_primary")} <ArrowRight className="h-4 w-4 rtl:rotate-180" /></Link>
            </Button>
